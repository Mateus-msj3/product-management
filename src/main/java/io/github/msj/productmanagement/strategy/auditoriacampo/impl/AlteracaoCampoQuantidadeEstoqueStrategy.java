...
public class AlteracaoCampoQuantidadeEstoqueStrategy implements AuditoriaCampoStrategy {
    @Override
    public boolean verificaAlteracao(Produto produtoAtual, Produto produtoAnterior) {
        return !Objects.equals(produtoAtual.getQuantidadeEstoque(), produtoAnterior.getQuantidadeEstoque());
    }

    @Override
    public AlteracaoCampoDTO obterAlteracao(Produto produtoAtual, Produto produtoAnterior) {
        return new AlteracaoCampoDTO("Quantidade Estoque", produtoAnterior.getQuantidadeEstoque().toString(), produtoAtual.getQuantidadeEstoque().toString());
    }
}
